...
import { Activity, FileImage, Scan, Settings2 } from "lucide-react";
import { useState } from "react";
import { Button } from "@/components/ui/button";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { analyzeXray } from "@/services/analysis-api";
                  ...
                  {!preview ? (
                    <label className="flex flex-col items-center justify-center w-full h-[400px] border border-dashed border-[var(--color-border)] hover:border-[var(--color-primary)]/50 rounded-2xl cursor-pointer bg-[var(--color-card)] transition-all group relative overflow-hidden">
                      ...
                      <div className="flex flex-col items-center justify-center text-center px-10 relative z-10">
                        ...
                        <p className="text-xs text-[var(--color-muted-foreground)] mt-2 px-8">
                          Use DICOM or high-resolution clinical imaging for best
                          results.
                        </p>
                      </div>
                      <input
                    ...
                    <div className="relative w-full h-[400px] rounded-2xl overflow-hidden border border-[var(--color-border)] group">
                      <img
                        src={preview}
                        alt="X-ray preview"
                        className="object-contain w-full h-full bg-[var(--color-surface-soft)]"
                      />
                    ...
                    {result ? (
                      ...
                      >
                        <div className="space-y-4">
                          ...
                          <div className="rounded-2xl overflow-hidden border border-[var(--color-border)] bg-[var(--color-surface-soft)]">
                            <img
                              src={result.imageUrl}
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <div className="p-6 bg-[var(--color-secondary)]/40 border border-[var(--color-border)] space-y-2 rounded-xl">
                            ...
                            <p className="text-base font-outfit font-semibold text-[var(--color-foreground)]">
                              {result.condition}
                            </p>
                          </div>
                          <div className="p-6 bg-[var(--color-secondary)]/40 border border-[var(--color-border)] space-y-2 rounded-xl">
                            ...
                            <p className="text-base font-outfit font-semibold text-[var(--color-accent)]">
                            ...
                            <div className="flex flex-wrap gap-2">
                              {result.detections.map((det, idx) => (
                                <div
                                  ...
                                  className="px-3 py-2 rounded-full border border-[var(--color-border)] bg-[var(--color-secondary)]/50 text-xs font-medium text-[var(--color-foreground)]"
                                >
                                  {det.class_name}{" "}
                      ...
                      <div className="flex-1 flex flex-col items-center justify-center opacity-40">
                        ...
                        <p className="text-sm font-semibold mt-8 text-center">
                          Awaiting image input
                        </p>
                        <p className="text-xs text-[var(--color-muted-foreground)] mt-2 text-center max-w-[240px]">
                          Upload an x-ray image to generate a structured
                          diagnostic report.
                        </p>
                      </div>
                    )}
